...
async def process_completed_window(window: dict, meeting_id: str | None = None, client=None) -> dict:
    ...
    if meeting_id:
        incident = state.store.get(meeting_id)
        if incident is not None:
            incident.draft_summary = step3["history"]
            await state.broadcast(incident, {
                "type": "recording.debug",
                "stage": "draft_updated",
                "message": "The in-memory and local meeting draft were updated.",
                "details": {
                    "saved_path": step3["saved_path"],
                    "segment_count": len(step3["history"]["segments"]),
                },
            })
